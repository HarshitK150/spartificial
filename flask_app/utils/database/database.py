...
#######################################################################################
    def createUser(self, email='me@email.com', password='password', role='user'):
        try:
            password_hash = self.onewayEncrypt(password)

            query = "INSERT INTO users (email, password, role) VALUES (%s, %s, %s);"
            parameters = (email, password_hash, role)
    ...
    def authenticate(self, email='me@email.com', password='password'):
        ...
        return {'success': 1} if result else {'failure': 2}
    ...
    def isOwner(self, email):
        ...
        parameters = (email,)
        role = self.query(query, parameters)
        
        if role and role[0]['role'] == 'owner':
            return True
